...
const ConfirmBooking = ({ navigation, params, dataItem }) => {
  ...
  const caculateTotalPrice = () => {
    ...
    if (params?.rooms && !_.isEmpty(params?.rooms)) {
      total["rooms"] = _.sumBy(params?.rooms, function (ex) {
        return Number(ex?.price) * Number(ex?.number_selected);
      });
      total["totalPrice"] += total["rooms"];
    }
  ...
  const detailOrder = () => {
    ...
    return (
      <View>
        {oneRow(global.language["Rooms"], totalPrice?.rooms)}
        {dash()}
