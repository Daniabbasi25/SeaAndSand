...
const ConfirmBookingBoat = ({ navigation, params, dataItem }) => {
  ...
  const caculateTotalPrice = () => {
    ...
    if (params?.extra_price && !_.isEmpty(params?.extra_price)) {
      total["extra_price"] = _.sumBy(params?.extra_price, function (ex) {
        if (ex?.number == "1") {
          return Number(ex?.price);
        }
      });
      if (!total["extra_price"]) total["extra_price"] = 0;
      total["totalPrice"] += total["extra_price"];
    }
    if (params?.ticket_types && !_.isEmpty(params?.ticket_types)) {
      _.forEach(params?.ticket_types, (e) => {
        let price = Number(e?.price) * Number(e?.number);
        total[e?.name] = price;
    ...
    if (params?.day) {
      total["totalPrice"] +=
        Number(dataItem?.price_per_day) * Number(params?.day);
      total["rentalPrice"] =
        Number(dataItem?.price_per_day) * Number(params?.day);
    }
    if (params?.hour) {
      total["totalPrice"] +=
        Number(dataItem?.price_per_hour) * Number(params?.hour);
      total["rentalPrice"] =
        Number(dataItem?.price_per_hour) * Number(params?.hour);
    }
  ...
  const handleListOption = () => {
    let list = [];
    _.forEach(params?.ticket_types, (e, i) => {
      list.push(
  ...
  const detailOrder = () => {
    ...
    return (
      <View>
        ...
        {oneRow(global.language["StartTime"], params?.start_time, true)}
        {(params?.hour || params?.day) &&
          oneRow(
            global.language["TimeReturn"],
            params?.hour
              ? params?.hour > 1
                ? params?.hour + ` ${global.language["Hours"]}`
                : params?.hour + ` ${global.language["Hour"]}`
              : params?.day &&
                  (params?.day > 1
                    ? params?.day + ` ${global.language["Days"]}`
                    : params?.day + ` ${global.language["Day"]}`),
            true
          )}
        {oneRow(
          global.language["RentalPrice"],
...
export default React.memo(ConfirmBookingBoat);
