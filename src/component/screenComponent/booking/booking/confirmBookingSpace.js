...
const ConfirmBookingSpace = ({ navigation, params, dataItem }) => {
  ...
  const caculateTotalPrice = () => {
    ...
    if (params?.start_date && params?.end_date) {
      ...
      if (dataItem?.sale_price && Number(dataItem?.sale_price)) {
        total["days"] = days * Number(dataItem?.sale_price);
        total["totalPrice"] += days * Number(dataItem?.sale_price);
      } else {
        total["days"] = days * Number(dataItem?.price);
        total["totalPrice"] += days * Number(dataItem?.price);
      }
    }
  ...
  const detailOrder = () => {
    ...
    return (
      <View>
        ...
        {oneRow(global.language["RentalPrice"], totalPrice?.days)}
        {handleListOption()}
        {dash()}
...
export default React.memo(ConfirmBookingSpace);
