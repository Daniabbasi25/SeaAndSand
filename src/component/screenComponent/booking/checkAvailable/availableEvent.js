...
import { defaultParamsCheckAvailableEvent } from "../../../../config/params/defaultParams";
...
const AvailableEvent = ({ type, id, navigation }) => {
  const [params, setParams] = useState(defaultParamsCheckAvailableEvent);
  ...
  const handleCheckAvailability = () => {
    ...
    getAvailableService(type, id, paramsPost).then((res) => {
      ...
      if (res && _.size(res) > 0) {
        ...
        newParams["ticket_types"] = res[0]["ticket_types"];
        ...
        <View style={styles.wrapPlusMinus}>
          ...
          <View style={styles.wrapNumber}>
            <TextApp primary bold>
              {params?.ticket_types &&
                _.size(params?.ticket_types) &&
                params?.ticket_types[index]["number"]}
            </TextApp>
          </View>
  ...
  const listOptionAvailable = () => {
    ...
    listAvailable &&
      ...
      _.forEach(listAvailable[0]?.ticket_types, (element, index) => {
  ...
  return (
    <View style={styles.wrapAvailableEvent}>
      {formCondition()}
...
export default React.memo(AvailableEvent);
